...
export const PlaybackControls: React.FC<PlaybackControlsProps> = ({
  ...
  return (
    <div
      id="playback-dock"
      className="w-full max-w-5xl mx-auto px-4 py-3 bg-[#0A0A0A]/95 backdrop-blur-xl border border-[#222] rounded-none shadow-2xl flex flex-col gap-3"
    >
      {/* Top Row: System Condition Breakdown & Playback Stats */}
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
        {/* Current Time Stamp */}
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white text-black font-mono font-bold text-xs uppercase tracking-wider shadow-[0_0_8px_rgba(255,255,255,0.3)]">
            <Clock className="w-3.5 h-3.5" />
            <span>T-{currentTick < 10 ? `0${currentTick}` : currentTick}</span>
          </div>

          <span className="text-[#888] hidden sm:inline text-[11px] font-mono uppercase tracking-wider">
            {currentTick === 0
        ...
        <div className="flex items-center gap-2 font-mono">
          <div className="flex items-center gap-1.5 px-2 py-0.5 border border-[#333] text-[#00F0FF] text-[11px] font-bold uppercase">
            <div className="w-1.5 h-1.5 rounded-full bg-[#00F0FF] shadow-[0_0_6px_#00F0FF]" />
            <span>{conditionCounts.operational} OPERATIONAL</span>
          </div>

          <div className="flex items-center gap-1.5 px-2 py-0.5 border border-[#333] text-[#FFC107] text-[11px] font-bold uppercase">
      ...
      <div className="flex items-center gap-3 w-full">
        <span className="text-[11px] text-[#666] font-mono whitespace-nowrap uppercase font-bold">T-00</span>
        ...
        <div className="relative flex-1 flex items-center">
          <input
            ...
            className="w-full h-2 bg-[#1A1A1A] rounded-none appearance-none cursor-pointer accent-white hover:accent-[#FF003C] transition-all focus:outline-none"
          />
          ...
          <div className="absolute left-0 right-0 top-3 flex justify-between pointer-events-none px-1">
            {snapshots.map((snap) => (
              <div key={snap.time} className="flex flex-col items-center">
                <div
                  className={`w-1 h-1.5 ${
                    snap.time === currentTick ? 'bg-white shadow-[0_0_8px_#FFF]' : 'bg-[#333]'
                  }`}
                />
                <span
                  className={`text-[9px] font-mono mt-0.5 ${
                    snap.time === currentTick ? 'text-white font-bold' : 'text-[#555]'
                  }`}
                >
                  {snap.time}
                </span>
        ...
        <span className="text-[11px] text-[#666] font-mono whitespace-nowrap uppercase font-bold">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-[#222]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-[#666] font-mono uppercase font-bold mr-1">Speed:</span>
          {[0.5, 1, 2].map((speed) => (
            <button
              key={speed}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="p-1.5 text-[#AAA] hover:text-white hover:bg-[#222] disabled:opacity-30 transition-colors"
          ...
          <button
            ...
            className="flex items-center justify-center w-9 h-9 bg-white hover:bg-[#FF003C] hover:text-white active:scale-95 text-black font-black shadow-[0_0_12px_rgba(255,255,255,0.3)] transition-all"
        ...
        <div className="text-[11px] text-[#666] font-mono uppercase font-bold hidden md:block">
          {isPlaying ? (
